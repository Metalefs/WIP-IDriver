<?php
    ...
    class Subject{
        ...
        private function createSubject($school_schoolId,$subjectName,$admin_account_id,$description,$introduction){
            $dBConnection = dbConnect();
            $query = "INSERT INTO subject (subjectId,school_schoolId,subjectName,admin_account_id,description,introduction) VALUES ($school_schoolId,'$subjectName',$admin_account_id,$description,$introduction);"; //Create a new subject in the database
            $queryResult = $dBConnection->query($query);
        }
        
        function __construct($subjectId){
            $dBConnection = dbConnect();
            $query = "select subjectId,school_schoolId,subjectName,creationDate,admin_account_id,description,introduction from `subject` where subjectId= '$subjectId';"; //Get all subject data
            $queryResult = $dBConnection->query($query);
